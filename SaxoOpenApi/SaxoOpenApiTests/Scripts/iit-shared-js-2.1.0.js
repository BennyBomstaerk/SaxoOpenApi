  ...
  */
  function convertFetchSuccess(result) {
    if (result.status < 200 || result.status > 299) {
      _log2.warn(LOG_AREA, 'non-200-299 okay response', result);
      return convertFetchResponse(result).then(rejectPromise);
    }
  ...
  function formatPricePartsFraction(parts, numberFormatting, value, decimals, formatFlags, numeratorDecimals) {
    ...
    if (formatFlags.ModernFractions) {
      ...
      if (formatFlags.AdjustFractions && numeratorText.length < denominatorText.length) {
        numeratorText = rightAdjust(numeratorText, denominatorText.length, NO_BREAK_SPACE);
      }
  ...
  function generateTransportCall(method) {
    return function (serviceGroup, urlTemplate, templateArgs, options) {
      ...
      if (templateArgs) {
        ...
        url = _utilsString.format(urlTemplate, urlEncodedTemplateArgs);
      } else {
        url = urlTemplate;
      }
      if (this.language) {
    ...
    if (result && result.status === 401) {
      if (this.auth.getExpiry() > Date.now()) {
        ...
        if (this.state & STATE_WAITING && this.retries === 0 && this.maxRetryCount > 0) {
          // if we aren't refreshing or retrying, refresh the token
          _log2.error(LOG_AREA, 'Received an auth error, auth is not expired - attempting refresh.', result);
          this.refreshOpenApiToken();
        } else {
          _log2.info(LOG_AREA, 'Received an auth error, currently trying to refresh.', result);
        }
      } else {
  ...
  function runBatchCall(serviceGroup, callList) {
    ...
    for (var i = 0, call = undefined; call = callList[i]; i++) {
      ...
      subRequests.push({
        method: call.method,
        headers: headers,
        url: this.basePath + serviceGroup + '/' + _utilsString.format(call.urlTemplate, call.urlArgs),
        data: body
      });
    }
    if (!authToken) {
...
iit_openapi_streaming_subscription = function (exports, _utilsObject, _log) {
  ...
  */
  Subscription.prototype.onStreamingData = function (message) {
    ...
    // the message.Data property can be an array or a single datum
    var dataList = Array.isArray(message.Data) ? message.Data : [message.Data];
    try {
      this.onUpdate(dataList, message);
    } catch (error) {
...
iit_openapi_streaming_streaming = function (exports, _microEmitter, _subscription, _heartbeater, _orphanFinder, _log) {
  ...
  */
  ...
  function setNewContextId() {
    var contextId = CONTEXT_ID_PREFIX + Date.now();
    this.contextId = contextId;
    for (var i = 0, subscription = undefined; subscription = this.subscriptions[i]; i++) {
  ...
  * @param {string} url - The name of the resource to subscribe to, e.g. /v1/infoprices/subscriptions/active.
  ...
  * @param {function} onUpdate - A callback function that is invoked when an initial snapshot is received.
  ...
  */
  ...
  exports = Streaming;
  return exports;
}({}, iit_micro_emitter, iit_openapi_streaming_subscription, iit_openapi_streaming_heartbeater, iit_openapi_streaming_orphan_finder, iit_log);
