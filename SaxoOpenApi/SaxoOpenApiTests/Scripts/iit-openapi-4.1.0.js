...
var iit = (function () { 'use strict';

   /**
    * @module iit/micro-emitter
    * @ignore
    */
   ...
   function mixinEmitter(target) {
   	...
   	target.on = function (eventType, onFunction, that) {
   		...
   		eventSubscribers.push({ onFunction: onFunction, that: that });
   	};
   ...
   function convertFetchResponse(result) {
   	...
   	if (contentType && contentType.indexOf("application/json") > -1) {
   		return result.text().then(function (text) {
   			try {
   				return { response: JSON.parse(text), status: result.status, headers: result.headers };
   			} catch (e) {
   				log.error(LOG_AREA, "Received a JSON response from OpenApi that could not be parsed", { text: text, response: result });
   				return { response: text, status: result.status, headers: result.headers };
   			}
   		});
   	...
   	return Promise.resolve({ status: result.status, headers: result.headers });
   }
   ...
   function localFetch(method, url, options) {
   	...
   	return fetch(url, { headers: headers, method: method, body: body, credentials: credentials }).then(convertFetchSuccess, convertFetchReject);
   }
    ...
    * @returns {string}
    */
   function formatTemplate(sTemplate, args) {
   ...
   function formatUrl(urlTemplate, templateArgs, queryParams) {
   	...
   	if (templateArgs) {
   		...
   		url = formatTemplate(urlTemplate, urlEncodedTemplateArgs);
   	} else {
   		url = urlTemplate;
   	}
   ...
   var utils_string = Object.freeze({
   ...
   */
   ...
   var utils_object = Object.freeze({
   	extend: extend
   });
   ...
   */
   ...
   var utils_function = Object.freeze({
   ...
   var TRANSITIONING_STATES = STATE_SUBSCRIBE_REQUESTED | STATE_UNSUBSCRIBE_REQUESTED;

   var ACTION_SUBSCRIBE = 0x20;
   var ACTION_UNSUBSCRIBE = 0x40;

   var DEFAULT_REFRESH_RATE_MS = 1000;
   var MIN_REFRESH_RATE_MS = 100;

   var LOG_AREA$6 = "Subscription";
   ...
   function subscribe() {
   	...
   	log.debug(LOG_AREA$6, "starting..", { serviceGroup: this.serviceGroup, url: this.url });
   ...
   function unsubscribe() {
   	...
   	this.transport.delete(this.serviceGroup, this.url + "/{contextId}/{referenceId}", { contextId: this.streamingContextId, referenceId: this.referenceId }).then(onUnsubscribeSuccess.bind(this, referenceId)).catch(onUnsubscribeError.bind(this, referenceId));
   }
   		...
   		case ACTION_SUBSCRIBE:
   			switch (this.currentState) {
   				...
   				case STATE_UNSUBSCRIBED:
   					...
   					break;
   				default:
   					log.error(LOG_AREA$6, "unanticipated state", { state: this.currentState, action: action });
   			}
   			break;
   ...
   function onSubscribeSuccess(referenceId, result) {
   	...
   	if (referenceId !== this.referenceId) {
   		log.warn(LOG_AREA$6, "Received an Ok subscribe response for subscribing a subscription that has afterwards been reset - ignoring");
   	...
   	if (this.inactivityTimeout === 0) {
   		log.warn(LOG_AREA$6, "inactivity timeout is 0 - interpretting as never timeout. Remove warning if normal.", result);
   	}
   ...
   function onSubscribeError(referenceId, response) {
   	if (referenceId !== this.referenceId) {
   		log.debug(LOG_AREA$6, "Received an error response for subscribing a subscription that has afterwards been reset - ignoring");
   		return;
   	}

   	this.currentState = STATE_UNSUBSCRIBED;
   	log.error(LOG_AREA$6, "An error occurred subscribing", { response: response, url: this.url });
   ...
   function onUnsubscribeError(referenceId, response) {
   	if (referenceId !== this.referenceId) {
   		log.error(LOG_AREA$6, "Received an error response for unsubscribing a subscription that has afterwards been reset - ignoring");
   		return;
   	}

   	this.currentState = STATE_UNSUBSCRIBED;
   	log.error(LOG_AREA$6, "An error occurred unsubscribing", { response: response, url: this.url });
   	onReadyToPerformNextAction.call(this);
   }
     ...
     */
   	...
   	if (!this.subscriptionData.RefreshRate) {
   		...
   		if (this.subscriptionData.RefreshRate < MIN_REFRESH_RATE_MS) {
   			log.warn(LOG_AREA$6, "Low refresh rate. This has been rounded up to the minimum.", { minimumRate: MIN_REFRESH_RATE_MS });
   ...
   Subscription.prototype.reset = function () {
   	...
   	switch (this.currentState) {
   		...
   		case STATE_SUBSCRIBED:
   			break;
   		default:
   			log.error(LOG_AREA$6, "reset was called but subscription is in an unknown state");
   			return;
   	}
   ...
   Subscription.prototype.onStreamingData = function (message) {
   	...
   	switch (this.currentState) {
   		...
   		case STATE_SUBSCRIBED:
   			break;
   		default:
   			log.error(LOG_AREA$6, "unanticipated state", this.currentState);
   	}

   	try {
   		this.onUpdate(message, this.UPDATE_TYPE_DELTA);
   	} catch (error) {
   		log.error(LOG_AREA$6, "exception occurred sending data update", error);
   	}
   };
   ...
   var LOG_AREA$5 = "StreamingOrphanFinder";
   ...
   StreamingOrphanFinder.prototype.update = function () {
   	...
   	var now = Date.now(),
   	    oldNextUpdateIn = this.nextUpdateTime - now,
   	    // old next Update In
   	newNextUpdateIn = Infinity,
   	    // new oldNextUpdateIn
   	foundNextUpdate = false,
   	...
   	if (oldNextUpdateIn < -MAX_UPDATE_DELAY) {
   		...
   		log.warn(LOG_AREA$5, "update occurred much later than requested, assuming wake from sleep and will retry", oldNextUpdateIn);
   	...
   	for (var i = 0, subscription; subscription = orphanedSubscriptions[i]; i++) {
   ...
   function onConnectionStateChanged(change) {
   	...
   	log.info(LOG_AREA$4, "Connection state changed to ", { changedTo: this.READABLE_CONNECTION_STATE_MAP[this.connectionState], mechanism: signalRTransport && signalRTransport.name });
   	...
   	switch (this.connectionState) {
   		case this.CONNECTION_STATE_DISCONNECTED:
   			...
   			log.error(LOG_AREA$4, "connection disconnected");
   ...
   emitter.mixinTo(Streaming.prototype);
   ...
   /**
    * @module iit/openapi/transport/queue
    * @ignore
    */
   ...
   function runBatchCall(serviceGroup, callList) {
   	...
   	this.transport.post(serviceGroup, "batch", null, { headers: { "Content-Type": 'multipart/mixed; boundary="' + boundary + '"' }, body: content, cache: false }).then(batchCallSuccess.bind(this, callList)).catch(batchCallFailure.bind(this, callList));
   }
   ...
   function generateTransportCall(method) {
     return function (serviceGroup, urlTemplate, templateArgs, options) {
       ...
       url = formatUrl(urlTemplate, templateArgs, queryParams);
       ...
       return this.fetch(method, this.baseUrl + "/" + serviceGroup + "/" + url, { body: body, headers: headers, cache: cache, useXHttpMethodOverride: this.useXHttpMethodOverride });
     };
   }
   ...
   function onTokenChanged(token, expiry) {
   	...
   	if (elapse <= 0) {
   		...
   		if (elapse < 0) {
   			log.warn(LOG_AREA$2, "token has changed, but expiry is less than the token refresh margin.", { expiry: expiry, tokenRefreshMarginMs: this.tokenRefreshMarginMs });
   			elapse = 0;
   		}
   ...
   emitter.mixinTo(TransportAuth.prototype);
   ...
   TransportAuth.prototype.onTokenInvalid = function () {
   	...
   	if (this.state & STATE_WAITING && this.retries === 0 && this.maxRetryCount > 0) {
   		log.error(LOG_AREA$2, "Token is invalid, refresh has not been attempted (may still be valid or timeout not fired).");
   		this.refreshOpenApiToken();
   	} else {
    ...
    * @namespace iit.priceFormatOptions
    * @example
    * var priceFormatOptions = iit.priceFormatOptions;
   ...
   */
   ...
   /**
    * @module iit/number-formatting/format
    * @ignore
    */
...
//# sourceMappingURL=iit-openapi-4.1.0.js.map